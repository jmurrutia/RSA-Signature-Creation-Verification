...
def getFileSig(fileName, privKey):
	...
	# you implemented.
		dataSig = digSig(privKey, dataHash)
...
def verifyFileSig(fileName, pubKey, signature):
	...
	# order to verify the file signature
		dataBool = verifySig(dataHash, signature, pubKey)
...
def saveSig(fileName, signature):
	...
	# by fileName)
	tupValue = signature[0]
	convertedTup = str(tupValue)
	with open(fileName, 'w') as sigFile:
		sigFile.write(convertedTup)
...
def loadSig(fileName):
	...
	# element tuple
	with open(fileName, 'r') as sig:
		stringToConvert = sig.read()
		sigToInteger = int(stringToConvert) #int(stringToConvert, 16)
		tup = (sigToInteger,)
	return tup
...
def main():
	...
	# We are verifying the signature
		...
		#pass
		loadedSig = loadSig(sigFileName)
		value = verifyFileSig(inputFileName, key, loadedSig)
		if value == True:
